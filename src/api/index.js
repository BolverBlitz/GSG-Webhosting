...
fs.readdir("./src/api", function(err, filenames) {
  ...
  for (i = 0; i < filenames.length; i++) { 
    if(filenames[i].startsWith("disabled.")){
      console.log('[P] \x1b[36m[WA]\x1b[0m',`Skipped ${filenames[i].slice(9, filenames[i].length-3)} because its disabled`)
    }else if(filenames[i].endsWith(".js")){
      ...
      if(typeof(iimport.PluginRequirements) === "undefined" || typeof(iimport.PluginName) === "undefined" || typeof(iimport.PluginVersion) === "undefined" || typeof(iimport.PluginAuthor) === "undefined" || typeof(iimport.PluginDocs) === "undefined" || typeof(iimport.router) === "undefined"){
        console.log('[P] \x1b[31m[ER]\x1b[0m',`Skipped ${filenames[i].slice(0, filenames[i].length-3)} because there are missing exports!`)
      }else{
        ...
        if(!PluginRequirementsFailed){
          ...
          console.log('[P] \x1b[32m[OK]\x1b[0m',`Loaded ${filenames[i].slice(0, filenames[i].length-3)}`)
        }else{
          console.log('[P] \x1b[31m[ER]\x1b[0m',`Plugin ${filenames[i].slice(0, filenames[i].length-3)}} requires following plugins [${iimport.PluginRequirements}] and at least one is missing!`)
        }
      }
    }else{
      console.log('[P] \x1b[31m[ER]\x1b[0m',`Unknown file was skipped ${filenames[i]}`)
    }
  }
})

console.log('[P] \x1b[32m[OK]\x1b[0m',`Loaded ${plugins.length} Plugins.`)
